...
Imported photos are held as browser object URLs for the current session only. They are never uploaded or processed remotely. Removing an imported photo, adding more photos, or clearing the project invalidates the current layout and placements so a new layout can be generated safely. **Clear All** returns the app to its start screen and releases the local photo resources.
